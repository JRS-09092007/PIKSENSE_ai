    ...
    <div className="pb-32 lg:pb-12 px-4 sm:px-6 pt-6 max-w-4xl mx-auto animate-slide-up space-y-6">
      ...
      <div>
        <h1 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight font-heading flex items-center gap-2">
          <Users className="text-primary-400" /> {t('experts_title')}
        </h1>
        <p className="text-sm text-surface-400">{t('experts_subtitle')}</p>
      </div>
        ...
        {filtered.map(expert => (
          <div key={expert.id} className="glass-card p-6 card-hover border border-white/8 space-y-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="font-extrabold text-white text-base font-heading"><T text={expert.name} /></h3>
                    {expert.verified && (
                  ...
                  <p className="text-xs text-surface-400 mt-0.5"><T text={expert.shop} /></p>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between text-xs text-surface-400 pt-1">
              <span className="flex items-center gap-1"><MapPin size={13} className="text-primary-400" /> <T text={expert.region} /></span>
              <span className="flex items-center gap-1.5 text-primary-300 font-semibold">
                <span className="w-2 h-2 rounded-full bg-primary-400 animate-ping" /> {t('ai_agronomist_active')}
              </span>
            </div>
            ...
            <div className="flex gap-2 pt-2 border-t border-white/5">
              <a href={`tel:${expert.contact}`}
                ...
                <Phone size={15} /> {t('talk_to_expert')}
              </a>
              <button
                ...
                {sentScans.includes(expert.id) ? (
                  ...
                  <><Send size={15} /> {t('send_to_agronomist')}</>
                )}
              </button>
            </div>
          </div>
      ...
      {filtered.length === 0 && (
        <div className="text-center py-16 glass-card border border-white/5">
          ...
          <p className="text-surface-300 font-medium">{t('no_data')}</p>
